...
namespace Assets.Code.Com.HappyBanana.OurAshes.Tactics.Main.Mvcs.QSorties.Frames.Requests.Impls
{
        ...
        : DefaultRequestImpl, IUnitWeaponGearIDModRequest
    {
        ...
        public UnitWeaponGearIDModRequestImpl(UnitID unitID, WeaponGearID weaponGearID, bool isAdd, int index, RequestType requestType)
            : base(requestType)
        {
            this.index = index;
            this.unitID = unitID;
        ...
        {
            return string.Format("{0}: UnitID:{1}, Index:{2}, WeaponGearID:{3}, IsAdd:{4}",
                this.GetType().Name, this.unitID, this.index, this.weaponGearID, this.isAdd);
        }
        ...
        /// <inheritdoc/>
        bool IUnitWeaponGearIDModRequest.IsAdd()
        {
            return isAdd;
        }
