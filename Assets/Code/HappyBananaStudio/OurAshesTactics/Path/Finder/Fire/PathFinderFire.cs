...
using Assets.Code.HappyBananaStudio.OurAshesTactics.Path.Objects.Fire;
using System.Collections.Generic;
using System.Diagnostics;

namespace Assets.Code.HappyBananaStudio.OurAshesTactics.Path.Finder.Fire
{
    /// <summary>
    /// Todo
    /// </summary>
    ...
    : PathFinderAbstract
    {
        ...
        public PathFinderFire(ICubeCoordinates cubeCoordinatesStart)
        ...
        private ICubeCoordinates GetNextCubeCoordinates(int pathIndex, int distance, ICubeCoordinates cubeCoordinatesEnd)
        {
                ...
                updateZ)
            {
                ...
                int newXFireCost = (updateX && newX + roundY + roundZ == 0)
                    ? GameMapObjectManager.FindHexTileObjectFireCostFrom(new CubeCoordinates(newX, roundY, roundZ))
                    : -1;
                int newYFireCost = (updateY && roundX + newY + roundZ == 0)
                    ? GameMapObjectManager.FindHexTileObjectFireCostFrom(new CubeCoordinates(roundX, newY, roundZ))
                    : -1;
                int newZFireCost = (updateZ && roundX + roundY + newZ == 0)
                    ? GameMapObjectManager.FindHexTileObjectFireCostFrom(new CubeCoordinates(roundX, roundY, newZ))
                    : -1;
            ...
            {
                throw ArgumentExceptionUtil.Build("Unable to ?. Invalid Parameters. PathIndex=?, Distance=?, ? produced invalid ?",
                    this.GetType(), pathIndex, distance, cubeCoordinatesEnd, typeof(ICubeCoordinates));
            }
        }
