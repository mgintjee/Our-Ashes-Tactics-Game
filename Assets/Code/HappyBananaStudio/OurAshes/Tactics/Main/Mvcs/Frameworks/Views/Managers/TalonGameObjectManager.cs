/*
 * namespace Assets.Code.HappyBananaStudio.OurAshes.Tactics.Main.Mvcs.Frameworks.Views.Managers
{
    ...
    public class TalonGameObjectManager
    {
        ...
        // Todo: Store in some constants file
        private static readonly string talonGameObjectCollectionName = "talonGameObjectCollection";

        // Todo
        private static readonly IDictionary<TalonCallSign, GameObject> talonIdentificationReportGameObjectDictionary =
            new Dictionary<TalonCallSign, GameObject>();
        ...
        /// </param>
        public static void BuildTalonGameObject(ITalonReport talonReport)
        {
            if (talonReport != null)
            {
        ...
        private static void AttachMounts(ITalonMountsScript talonMountsScript, ITalonConstructionReport talonConstructionReport)
        {
            ...
            foreach (WeaponModelId weaponModelId in talonConstructionReport.GetWeaponModelIdList())
            {
                ...
                {
                    throw ExceptionUtil.Argument.Build("Unable to ?. WeaponMountIndex=? is out of bounds. Must be less than ?.",
            ...
            foreach (UtilityModelId utilityModelId in talonConstructionReport.GetUtilityModelIdList())
            {
                ...
                {
                    throw ExceptionUtil.Argument.Build("Unable to ?. UtilityMountIndex=? is out of bounds. Must be less than ?.",
        ...
        private static Vector3 GetLocalEulerAnglesFor(FactionId factionId)
        {
            switch (factionId)
            {
                ...
                default:
                    throw ExceptionUtil.Argument.Build("Unable to ?. ? is not supported.",
                        new StackFrame().GetMethod().Name, factionId);
            }
        }
...
if (talonConstructionReport != null)
{
    ...
    if (talonIdentificationReport != null)
    {
        ...
        if (talonGameObject != null)
        {
            ...
            {
                throw ExceptionUtil.Argument.Build("Unable to ?. Invalid Parameters. ? does not have the correct amount of values." +
                    "\n\t> Required=?, Actual=?.",
        ...
        {
            throw ExceptionUtil.Argument.Build("Unable to ?. Invalid Parameters. Talon ? is null.",
...
{
    throw ExceptionUtil.Argument.Build("Unable to ?. Invalid Parameters. Spawned Weapon ? is null.",
        new StackFrame().GetMethod().Name, typeof(GameObject));
}
}
