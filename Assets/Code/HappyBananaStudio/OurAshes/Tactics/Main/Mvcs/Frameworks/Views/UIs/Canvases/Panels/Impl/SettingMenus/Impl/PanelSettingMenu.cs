...
namespace Assets.Code.HappyBananaStudio.OurAshes.Tactics.Main.Mvcs.Frameworks.Views.UIs.Canvases.Panels.Impl.SettingMenus.Impl
{
    /// <summary>
    /// Todo
    /// </summary>
    public class PanelSettingMenu
        : AbstractPanel, IPanelSettingMenu
    {
        ...
        public class Builder
        {
            ...
            /// </summary>
            /// <returns></returns>
            public IPanelSettingMenu Build()
            {
                ISet<string> invalidReasons = this.IsInvalid();
                // Check that the set parameters are valid
                if (invalidReasons.Count == 0)
                {
                    PanelSettingMenu panelSettingMenu =
                        new GameObject(typeof(PanelSettingMenu).Name)
                        .AddComponent<PanelSettingMenu>();
                    // TODO: Store the GridDimensions in a const file
                    ((IPanel)panelSettingMenu).Initialize(this.canvasGridConvertor,
                        this.panelConfigurationReport,
                        new GridDimensions(1, 2),
                        this.parentTransform);
                    return panelSettingMenu;
                }
