namespace Assets.Code.HappyBananaStudio.OurAshes.Tactics.Main.Mvcs.Frameworks.Views.UIs.Canvases.Impl.TurnScrollers.Impl
{
    ...
    /// Todo
    /// </summary>
    public class CanvasTurnScroller
        : AbstractCanvas, ICanvasTurnScroller
    {
        /// <summary>
        /// Todo
        /// </summary>
        ...
        public class Builder
        {
            ...
            /// </summary>
            /// <returns></returns>
            public ICanvasTurnScroller Build()
            {
                ISet<string> invalidReasons = this.IsInvalid();
                // Check that the set parameters are valid
                if (invalidReasons.Count == 0)
                {
                    CanvasTurnScroller turnScroller = new GameObject(typeof(CanvasTurnScroller).Name)
                        .AddComponent<CanvasTurnScroller>();
                    turnScroller.GetTransform().SetParent(this.parentTransform);
                    turnScroller.GetTransform().localPosition = Vector3.zero;
                    turnScroller.GetTransform().localScale = Vector3.one;
                    turnScroller.SetCanvasConfigurationReport(this.canvasConfigurationReport);
                    turnScroller.LoadCanvasEntryWidgets();
                    return turnScroller;
                }
                else
                {
