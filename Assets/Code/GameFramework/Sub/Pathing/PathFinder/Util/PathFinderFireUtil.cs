...
public static class PathFinderFireUtil
{
    ...
    public static Dictionary<CubeCoordinates, PathObject> BeginPathfindingFor(CubeCoordinates tileCoordinatesStart, int mechTeam)
    {
        logger.Debug("PathFinding for FIRING from CubeCoordinates: ?, for MechTeam=?", tileCoordinatesStart, mechTeam);
        pathFinderFire = new PathFinderFire(tileCoordinatesStart, mechTeam);
    ...
    private static void DebugDrawPathLines(Dictionary<CubeCoordinates, PathObject> pathObjectDictionary)
    {
        foreach (CubeCoordinates cubeCoordinates in pathObjectDictionary.Keys)
        {
            ...
            foreach (CubeCoordinates cubeCoordinatesStep in pathObject.GetCubeCoordinatesStepList())
            {
                ...
                previousTilePosition.y = 5f;
                currentTilePosition.y = 5f;
                //Debug.DrawLine(previousTilePosition, currentTilePosition, Color.red, 100f);
