...
using Assets.Code.HappyBananaStudio.OurAshes.Tactics.Main.Mvcs.Talons.Models.Pathing.Finders.Abs;
...
namespace Assets.Code.HappyBananaStudio.OurAshes.Tactics.Main.Mvcs.Talons.Models.Pathing.Finders.Impl
{
    /// <summary>
    /// Todo
    /// </summary>
        ...
        : AbstractPathFinder
    {
        ...
        /// </param>
        private PathFinderFire(ICubeCoordinates cubeCoordinatesStart, int maxRange, float maxAccuracy)
        ...
        {
            ISet<ICubeCoordinates> allCubeCoordinatesEndSet = GameBoardManager.GetAllCubeCoordinatesSet();
            ISet<ICubeCoordinates> validCubeCoordinatesEndSet = new HashSet<ICubeCoordinates>();
            logger.Debug("BeginPathFinding for maxRange=?, maxAccuracy=?", this.maxRange, this.maxAccuracy);
            foreach (ICubeCoordinates cubeCoordinates in allCubeCoordinatesEndSet)
            {
                ...
                if (!this.cubeCoordinatesStart.Equals(cubeCoordinates))
                {
                    ...
                    if (hexTileObject != null)
                    {
                        ...
                        if (!hexTileReport.GetTalonCallSign().Equals(TalonCallSign.None))
                        {
                            logger.Debug("Valid ?", hexTileReport);
            ...
            foreach (ICubeCoordinates cubeCoordinates in validCubeCoordinatesEndSet)
            {
                ...
                {
                    IPathObject pathObject = new PathObjectFire(straightLinePath);
                ...
                {
                    logger.Debug("[?] is invalid. Range is further than Max=?",
                        string.Join(", ", straightLinePath), this.maxRange);
                }
            }
        }
        ...
        private ICubeCoordinates GetNextCubeCoordinates(int pathIndex, int distance, ICubeCoordinates cubeCoordinatesEnd)
        {
                ...
                updateZ)
            {
                ...
                float newXFireCost = (updateX && newX + roundY + roundZ == 0)
                    ? GameBoardManager.FindHexTileObjectFireCostFrom(
                        new CubeCoordinates(newX, roundY, roundZ))
                    : -1;
                float newYFireCost = (updateY && roundX + newY + roundZ == 0)
                    ? GameBoardManager.FindHexTileObjectFireCostFrom(
                        new CubeCoordinates(roundX, newY, roundZ))
                    : -1;
                float newZFireCost = (updateZ && roundX + roundY + newZ == 0)
                    ? GameBoardManager.FindHexTileObjectFireCostFrom(
                        new CubeCoordinates(roundX, roundY, newZ))
                    : -1;
        ...
        public class Builder
        {
            ...
            public IPathFinder Build()
            {
                ...
                if (invalidReasons.Count == 0)
                {
                    ...
                    return new PathFinderFire(this.startingCubeCoordinates, this.maxRange, this.maxAccuracy);
                }
                else
                {
