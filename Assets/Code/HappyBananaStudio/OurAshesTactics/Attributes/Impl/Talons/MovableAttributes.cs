...
namespace Assets.Code.HappyBananaStudio.OurAshesTactics.Attributes.Impl.Talons
{
        ...
        : IMovableAttributes
    {
        ...
        /// </param>
        public MovableAttributes(int movePoints, int turnPoints)
        {
        ...
        /// </param>
        private MovableAttributes(ICollection<IMovableAttributes> movableAttributeCollection)
        {
            int movePoints = 0;
            int turnPoints = 0;
        ...
        public class Builder
        {
            ...
            public IMovableAttributes Build()
            {
                ...
                if (invalidReasons.Count == 0)
                {
                    if (this.movableAttributesCollection != null)
                    {
                        ...
                        return new MovableAttributes(this.movableAttributesCollection);
                    }
                    else
                    {
                        // Instantiate a new Report
                        return new MovableAttributes(this.movePoints, this.turnPoints);
                    }
                }
                else
                {
            ...
            private HashSet<string> IsInvalid()
            {
                ...
                if (this.movableAttributesCollection == null)
                {
                    ...
                    {
                        argumentExceptionSet.Add("Collection: " + typeof(IMovableAttributes).Name + " has not been set");
                    }
                }
                else
                {
