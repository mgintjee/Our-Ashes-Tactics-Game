...
namespace HappyBananaStudio.OurAshesTactics.Mvc.Pathing.Finder.Impl
{
    ...
    : PathFinderAbstract
    {
        ...
        {
            HashSet<CubeCoordinates> allCubeCoordinatesEndSet = MapObjectManager.GetAllCubeCoordinatesSet();
        ...
        private CubeCoordinates GetNextCubeCoordinates(int pathIndex, int distance, CubeCoordinates cubeCoordinatesEnd)
        {
                ...
                updateZ)
            {
                ...
                int newXFireCost = (updateX && newX + roundY + roundZ == 0)
                    ? MapObjectManager.FindHexTileObjectFireCostFrom(new CubeCoordinates(newX, roundY, roundZ))
                    : -1;
                int newYFireCost = (updateY && roundX + newY + roundZ == 0)
                    ? MapObjectManager.FindHexTileObjectFireCostFrom(new CubeCoordinates(roundX, newY, roundZ))
                    : -1;
                int newZFireCost = (updateZ && roundX + roundY + newZ == 0)
                    ? MapObjectManager.FindHexTileObjectFireCostFrom(new CubeCoordinates(roundX, roundY, newZ))
                    : -1;
