...
namespace HappyBananaStudio.OurAshesTactics.Mvc.Model.Sub.Map.Impl
{
    ...
    : IMapObject
    {
        ...
        // Todo
        private readonly Dictionary<CubeCoordinates, IHexTileObject> cubeCoordinatesHexTileObjectDictionary;
        ...
        public MapObjectImpl(MapScript mapScript, MapConstructionReport mapConstructionReport)
        {
                ...
                mapConstructionReport != null)
            {
                ...
                MapObjectManager.SetMapObject(this);
            }
            else
            {
        ...
        /// <returns></returns>
        public int GetHexTileObjectFireCostFrom(CubeCoordinates cubeCoordinates)
        {
            if (cubeCoordinates != null)
            {
                HexTileInformationReport hexTileInformationReport = this.GetHexTileObjectFrom(cubeCoordinates).GetHexTileInformationReport();
                if (hexTileInformationReport != null)
                {
                    ...
                    {
                        throw new ArgumentException("Unable to FindHexTileObjectFireCostFrom. " + typeof(HexTileAttributes) + " is null.");
                    }
                }
                else
                {
                    throw new ArgumentException("Unable to FindHexTileObjectFireCostFrom. " + typeof(HexTileInformationReport) + " is null.");
                }
            }
            else
            {
                throw new ArgumentException("Unable to FindHexTileObjectFireCostFrom. Invalid Parameters");
            }
        }
        ...
        /// <param name="cubeCoordinates"></param>
        /// <returns></returns>
        public IHexTileObject GetHexTileObjectFrom(CubeCoordinates cubeCoordinates)
        {
            if (cubeCoordinates != null)
            {
                ...
                {
                    throw new ArgumentException("Unable to FindHexTileObjectFrom. " + cubeCoordinates + " is not tracked.");
                }
            }
            else
            {
                throw new ArgumentException("Unable to FindHexTileObjectFrom. Invalid Parameters");
            }
        }
        ...
        /// <param name="cubeCoordinates"></param>
        /// <returns></returns>
        public HashSet<CubeCoordinates> GetNeighborCubeCoordinates(CubeCoordinates cubeCoordinates)
        {
            if (cubeCoordinates != null)
            {
                IHexTileObject tileObject = this.GetHexTileObjectFrom(cubeCoordinates);
                if (tileObject != null)
                {
                    ...
                    if (hexTileInformationReport != null)
                    {
                        ...
                        {
                            throw new ArgumentException("Unable to GetNeighborCubeCoordinates. " + typeof(HexTileConstructionReport) + " is null.");
                        }
                    }
                    else
                    {
                        throw new ArgumentException("Unable to GetNeighborCubeCoordinates. " + typeof(HexTileInformationReport) + " is null.");
                    }
                }
                else
                {
                    throw new ArgumentException("Unable to GetNeighborCubeCoordinates. " + typeof(IHexTileObject) + " is null.");
                }
            }
            else
            {
        ...
        private HashSet<HexTileConstructionReport> BuildTileInfoReportSet()
        {
            ...
            HashSet<HexTileConstructionReport> hexTileInformationReportSet = HexTileConstructionReportGeneratorUtil
                .GenerateHexTileInformationReportSet(cubeCoordinatesSet,
                this.mapInformationReport.GetMapMirrored(),
                this.mapInformationReport.GetMapRadius());
