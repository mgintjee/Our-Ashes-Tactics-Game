    ...
    : PathFinderAbstract
{
    ...
    public PathFinderFire(CubeCoordinates cubeCoordinatesStart, TeamIdEnum teamId)
        : base(cubeCoordinatesStart)
    {
        this.mechTeam = teamId;
    }
    ...
    public override void BeginPathFinding()
    {
        ...
        foreach (CubeCoordinates cubeCoordinates in allCubeCoordinatesEndSet)
        {
            ...
            if (tileObject != null &&
                ...
                !tileObject.GetTileInfoReport().GetOccupyingMechObject().GetTeamId().Equals(this.mechTeam))
            {
                logger.Debug("For ?=?, Valid Target To Shoot At ?=? ", typeof(TeamIdEnum), this.mechTeam,
                    typeof(CubeCoordinates), tileObject.GetCubeCoordinates());
