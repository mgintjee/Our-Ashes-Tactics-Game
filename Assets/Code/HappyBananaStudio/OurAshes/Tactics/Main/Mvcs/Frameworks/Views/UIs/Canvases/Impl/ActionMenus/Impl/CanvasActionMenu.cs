namespace Assets.Code.HappyBananaStudio.OurAshes.Tactics.Main.Mvcs.Frameworks.Views.UIs.Canvases.Impl.ActionMenus.Impl
{
    ...
    /// Todo
    /// </summary>
    public class CanvasActionMenu
        : AbstractCanvas, ICanvasActionMenu
    {
        /// <summary>
        /// Todo
        /// </summary>
        ...
        public class Builder
        {
            ...
            /// </summary>
            /// <returns></returns>
            public ICanvasActionMenu Build()
            {
                ISet<string> invalidReasons = this.IsInvalid();
                // Check that the set parameters are valid
                if (invalidReasons.Count == 0)
                {
                    CanvasActionMenu canvasActionMenu = new GameObject(typeof(CanvasActionMenu).Name)
                        .AddComponent<CanvasActionMenu>();
                    canvasActionMenu.GetTransform().SetParent(this.parentTransform);
                    canvasActionMenu.GetTransform().localPosition = Vector3.zero;
                    canvasActionMenu.GetTransform().localScale = Vector3.one;
                    canvasActionMenu.SetCanvasConfigurationReport(this.canvasConfigurationReport);
                    canvasActionMenu.LoadCanvasEntryWidgets();
                    return canvasActionMenu;
                }
                else
                {
