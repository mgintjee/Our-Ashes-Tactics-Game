...
namespace Assets.Code.HappyBananaStudio.OurAshes.Tactics.Main.Mvcs.Frameworks.Views.UIs.Canvases.Panels.Impl.ScoreBoards.Impl
{
    ...
    public class PanelScoreBoard
        : AbstractPanel, IPanelScoreBoard
    {
        ...
        public class Builder
        {
            ...
            /// </summary>
            /// <returns></returns>
            public IPanelScoreBoard Build()
            {
                ISet<string> invalidReasons = this.IsInvalid();
                // Check that the set parameters are valid
                if (invalidReasons.Count == 0)
                {
                    PanelScoreBoard panelScoreBoard =
                        new GameObject(typeof(PanelScoreBoard).Name)
                        .AddComponent<PanelScoreBoard>();
                    // TODO: Store the GridDimensions in a const file
                    ((IPanel)panelScoreBoard).Initialize(this.canvasGridConvertor,
                        this.panelConfigurationReport,
                        new GridDimensions(1, 2),
                        this.parentTransform);
                    return panelScoreBoard;
                }
